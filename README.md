<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0d0f1a,50:1a1533,100:0d0f1a&height=230&section=header&text=Yash%20Mishra&fontSize=56&fontColor=ffffff&fontAlignY=35&desc=Deep%20Learning%20%E2%80%A2%20NLP%20Research%20%E2%80%A2%20Transformer%20Architectures&descAlignY=58&descSize=16&animation=fadeIn" width="100%"/>

<a href="https://git.io/typing-svg"><img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=600&size=22&duration=2600&pause=900&color=A78BFA&center=true&vCenter=true&width=750&lines=Building+Transformers+from+Scratch;Attention+Mechanisms+%26+Interpretability;Neural+Machine+Translation+Research;Published+Springer+Nature+Author;GATE+CSE+%2B+DA+2027+Aspirant" alt="Typing SVG" /></a>

<br/>

![](https://raw.githubusercontent.com/andreasbm/readme/master/assets/lines/rainbow.gif)

<sub>🧠 &nbsp; researching how machines learn language, one attention head at a time &nbsp; 🧠</sub>

</div>

<br/>

<div align="center">
<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=500&size=14&duration=3000&pause=500&color=7C3AED&center=true&vCenter=true&width=600&lines=%3E+import+torch.nn+as+nn;%3E+model+%3D+TransformerEncoderDecoder();%3E+model.train(patience%3D%E2%88%9E)" alt="code typing" />
</div>

<br/>

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:1a1533,100:0d0f1a&height=3&width=100%" width="100%"/>

<br/>

<div align="center">

### 📍 Quick Facts

</div>

<table align="center">
<tr>
<td>📍&nbsp; <b>Based in</b></td><td>New Delhi, India</td>
<td>🎓&nbsp; <b>Education</b></td><td>B.Tech CSE (AI &amp; ML)</td>
</tr>
<tr>
<td>🔬&nbsp; <b>Research</b></td><td>NLP &amp; Neural Machine Translation</td>
<td>📝&nbsp; <b>Publishing</b></td><td>Published Springer Nature Author</td>
</tr>
<tr>
<td>🧮&nbsp; <b>GATE 2026</b></td><td colspan="3">Qualified <b>both</b> GATE CSE &amp; GATE DA — All India Rank ~6.5K</td>
</tr>
<tr>
<td>🎯&nbsp; <b>GATE 2027</b></td><td colspan="3">Now preparing for <b>GATE CSE + GATE DA 2027</b> — chasing a top-tier rank for IIT/IISc AI-ML admits</td>
</tr>
<tr>
<td>🌱&nbsp; <b>Learning</b></td><td colspan="3">Advanced LLM systems, RAG pipelines &amp; efficient Transformers</td>
</tr>
<tr>
<td>🤝&nbsp; <b>Open to</b></td><td colspan="3">Research collaborations &amp; open-source contributions</td>
</tr>
</table>

<br/>

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:1a1533,100:0d0f1a&height=3&width=100%" width="100%"/>

<br/>

## ✦ About

I'm a **B.Tech CSE (AI & ML)** graduate on a dedicated research year, deep-diving into **Deep Learning** and **Natural Language Processing** while preparing for **GATE CSE & GATE DA 2027**. I already qualified both exams this year with an AIR of ~6.5K — now I'm back for a sharper rank and a stronger AI/ML graduate profile.

My core research obsession is **Transformer architectures** — building them from scratch, probing what attention heads actually learn, and pushing translation quality for **mid-resource language pairs** like English–Hindi. I don't just call `.fit()` on pretrained models; I like understanding *why* the math works before I trust the output.

<div align="center">

<img src="https://cdn.jsdelivr.net/gh/yash-mishraa/yash-mishraa@main/assets/neural-network-animated.svg" width="100%"/>

</div>

<br/>

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:1a1533,100:0d0f1a&height=3&width=100%" width="100%"/>

<br/>

## 🎯 GATE CSE + DA 2027 — Prep in Progress

<div align="center">

![GATE CSE](https://img.shields.io/badge/GATE_CSE_2027-In_Progress-A78BFA?style=for-the-badge&logo=bookstack&logoColor=white)
![GATE DA](https://img.shields.io/badge/GATE_DA_2027-In_Progress-7C3AED?style=for-the-badge&logo=bookstack&logoColor=white)
![2026 Result](https://img.shields.io/badge/2026_AIR-~6.5K_(Qualified_Both)-1a1533?style=for-the-badge&logo=checkmarx&logoColor=white)

</div>

Grinding through probability & statistics, linear algebra, DSA, and CS/DA core theory — with a goal of an AIR strong enough for **IIT / IISc AI-ML MTech or research programs**. Exhaustive LaTeX notes, spaced revision, and PYQ-driven practice power the whole grind (yes, including a [self-built tracker](#-featured-projects) for it).

<br/>

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:1a1533,100:0d0f1a&height=3&width=100%" width="100%"/>

<br/>

## ⚡ Current Focus

<table>
<tr>
<td width="50%" valign="top">

**🔬 Research**
- Transformer interpretability for mid-resource NMT
- BPE tokenization efficiency vs. WordLevel tokenization
- English–Hindi translation on the IIT Bombay corpus
- Attention-head entropy & alignment visualization

</td>
<td width="50%" valign="top">

**🧠 Deep Learning Study**
- Transformer architectures from first principles
- LLM fine-tuning, alignment & scaling laws
- Efficient attention / long-context mechanisms
- Retrieval-Augmented Generation (RAG)

</td>
</tr>
<tr>
<td width="50%" valign="top">

**📖 GATE 2027 Prep**
- CSE + DA core subjects, DSA, probability & statistics
- Linear algebra & optimization for ML
- PYQ-driven, tracker-backed revision system

</td>
<td width="50%" valign="top">

**🛠 Building**
- [S2S Transformer](#-featured-projects) — from-scratch NMT architecture
- [ComicCrafter AI](#-featured-projects) — LLM + diffusion pipeline
- [gate-tracker](https://github.com/yash-mishraa/gate-tracker) — GATE prep tracker

</td>
</tr>
</table>

<br/>

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:1a1533,100:0d0f1a&height=3&width=100%" width="100%"/>

<br/>

## 🧠 Research Interests

<div align="center">

| Domain | Focus |
|---|---|
| **Neural Machine Translation** | From-scratch Transformer seq2seq, BPE tokenization, cross-lingual transfer |
| **Attention Mechanisms** | Layer-wise interpretability, entropy analysis, alignment visualization |
| **Large Language Models** | Architecture internals, fine-tuning, prompt sensitivity |
| **Cross-lingual NLP** | English–Hindi translation, low/mid-resource language challenges |
| **Deep Learning Theory** | Optimization, generalization, efficient inference |

</div>

<br/>

<div align="center">
<img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=500&size=15&duration=2800&pause=800&color=A78BFA&center=true&vCenter=true&width=600&lines=softmax(QK%5ET+%2F+%E2%88%9Ad_k)V;loss+%3D+CrossEntropy(logits%2C+targets);attention.visualize(layer%3D6%2C+head%3D3)" alt="math typing" />
</div>

<br/>

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:1a1533,100:0d0f1a&height=3&width=100%" width="100%"/>

<br/>

## 📖 Publications

### 📘 Published Springer Nature Book Chapter

**A Transparent Transformer Framework for English–Hindi Neural Machine Translation: Design and Preliminary Evaluation**

**Publisher:** Springer Nature &nbsp;·&nbsp; **Conference:** ICTCS 2025 &nbsp;·&nbsp; **Series:** Lecture Notes in Networks and Systems (LNNS)

[![Springer](https://img.shields.io/badge/-Read_on_Springer-0d1117?style=for-the-badge&logo=springer&logoColor=white)](https://link.springer.com/chapter/10.1007/978-3-032-19672-9_41)

---

### 📄 Extended Journal Version

An extended version with additional experiments on tokenization efficiency, uncertainty analysis, and attention interpretability.

[![Research Square](https://img.shields.io/badge/-DOI%3A_10.21203%2Frs.3.rs--9327858%2Fv1-0d1117?style=for-the-badge&logo=readdotcv&logoColor=white)](https://doi.org/10.21203/rs.3.rs-9327858/v1)

<br/>

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:1a1533,100:0d0f1a&height=3&width=100%" width="100%"/>

<br/>

## 🏆 Research Impact

- 📘 Published author in **Springer Nature (ICTCS 2025)**
- 📄 Extended journal version available on Research Square
- 🤖 Built a Transformer architecture from scratch for English–Hindi Neural Machine Translation
- 🧠 Research interests: Transformers · LLMs · NLP · Explainable AI · Efficient NMT

<br/>

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:1a1533,100:0d0f1a&height=3&width=100%" width="100%"/>

<br/>

## 🏗 Featured Projects

<table>
<tr>
<td width="33%" valign="top">

📘 **Flagship Research Project**

### 🤖 S2S Transformer

**From-scratch NMT architecture**

An encoder-decoder Transformer built entirely from scratch (co-developed) — multi-head attention, sinusoidal positional encoding, causal masking — for English–Hindi NMT. Publication-backed.

`PyTorch` `Python` `BPE Tokenization` `IITB Corpus`

[![Repo](https://img.shields.io/badge/-Repository-0d1117?style=for-the-badge&logo=github)](https://github.com/saurabhkumar9901/S2S-Tranformer-Architecture-)

</td>
<td width="33%" valign="top">

🎨 **Generative AI Project**

### 🎨 ComicCrafter AI

**Generative AI storytelling pipeline**

A prompt-to-comic pipeline (co-developed) combining a locally hosted LLM for structured story generation with a fine-tuned Stable Diffusion model for panel illustration.

`Python` `Gradio` `LLM` `ComfyUI` `Stable Diffusion`

[![Repo](https://img.shields.io/badge/-Repository-0d1117?style=for-the-badge&logo=github)](https://github.com/saurabhkumar9901/ComicCrafterAI)

</td>
<td width="33%" valign="top">

⭐ **Systems Project**

### 🚀 CodeTrail

**A complete DSA learning ecosystem**

Structured roadmap through 97 algorithmic patterns and 580+ curated problems, with cloud-synced progress tracking and analytics.

`Next.js 15` `React 19` `TypeScript` `Supabase`

[![Repo](https://img.shields.io/badge/-Repository-0d1117?style=for-the-badge&logo=github)](https://github.com/yash-mishraa/CodeTrail)
[![Demo](https://img.shields.io/badge/-Live_Demo-7C3AED?style=for-the-badge&logo=vercel&logoColor=white)](https://codetrailapp.site)

</td>
</tr>
</table>

<br/>

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:1a1533,100:0d0f1a&height=3&width=100%" width="100%"/>

<br/>

## 🔧 Skills & Tech Stack

<details open>
<summary><b>🧠 Deep Learning &amp; ML</b></summary><br/>

![PyTorch](https://img.shields.io/badge/PyTorch-0d1117?style=flat-square&logo=pytorch&logoColor=EE4C2C)
![TensorFlow](https://img.shields.io/badge/TensorFlow-0d1117?style=flat-square&logo=tensorflow&logoColor=FF6F00)
![HuggingFace](https://img.shields.io/badge/HuggingFace-0d1117?style=flat-square&logo=huggingface&logoColor=FFD21E)
![scikit-learn](https://img.shields.io/badge/scikit--learn-0d1117?style=flat-square&logo=scikit-learn&logoColor=F7931E)
![NumPy](https://img.shields.io/badge/NumPy-0d1117?style=flat-square&logo=numpy&logoColor=013243)
![Pandas](https://img.shields.io/badge/Pandas-0d1117?style=flat-square&logo=pandas&logoColor=150458)
![OpenCV](https://img.shields.io/badge/OpenCV-0d1117?style=flat-square&logo=opencv&logoColor=5C3EE8)

</details>

<details open>
<summary><b>🤖 Generative AI / LLM</b></summary><br/>

![LangChain](https://img.shields.io/badge/LangChain-0d1117?style=flat-square&logo=langchain&logoColor=white)
![OpenAI](https://img.shields.io/badge/OpenAI-0d1117?style=flat-square&logo=openai&logoColor=white)
![Stable Diffusion](https://img.shields.io/badge/Stable_Diffusion-0d1117?style=flat-square&logo=stability-ai&logoColor=white)
![Gradio](https://img.shields.io/badge/Gradio-0d1117?style=flat-square&logo=gradio&logoColor=FF7C00)

</details>

<details>
<summary><b>Languages</b></summary><br/>

![Python](https://img.shields.io/badge/Python-0d1117?style=flat-square&logo=python&logoColor=3776AB)
![TypeScript](https://img.shields.io/badge/TypeScript-0d1117?style=flat-square&logo=typescript&logoColor=3178C6)
![JavaScript](https://img.shields.io/badge/JavaScript-0d1117?style=flat-square&logo=javascript&logoColor=F7DF1E)
![C++](https://img.shields.io/badge/C++-0d1117?style=flat-square&logo=cplusplus&logoColor=00599C)
![Java](https://img.shields.io/badge/Java-0d1117?style=flat-square&logo=openjdk&logoColor=ED8B00)

</details>

<details>
<summary><b>Full-Stack (for shipping AI products)</b></summary><br/>

![React](https://img.shields.io/badge/React-0d1117?style=flat-square&logo=react&logoColor=61DAFB)
![Next.js](https://img.shields.io/badge/Next.js-0d1117?style=flat-square&logo=nextdotjs&logoColor=ffffff)
![Tailwind CSS](https://img.shields.io/badge/Tailwind_CSS-0d1117?style=flat-square&logo=tailwindcss&logoColor=38B2AC)
![FastAPI](https://img.shields.io/badge/FastAPI-0d1117?style=flat-square&logo=fastapi&logoColor=009688)
![Node.js](https://img.shields.io/badge/Node.js-0d1117?style=flat-square&logo=nodedotjs&logoColor=339933)
![Flask](https://img.shields.io/badge/Flask-0d1117?style=flat-square&logo=flask&logoColor=ffffff)

</details>

<details>
<summary><b>Databases &amp; Cloud</b></summary><br/>

![PostgreSQL](https://img.shields.io/badge/PostgreSQL-0d1117?style=flat-square&logo=postgresql&logoColor=4169E1)
![MongoDB](https://img.shields.io/badge/MongoDB-0d1117?style=flat-square&logo=mongodb&logoColor=47A248)
![Firebase](https://img.shields.io/badge/Firebase-0d1117?style=flat-square&logo=firebase&logoColor=FFCA28)
![AWS](https://img.shields.io/badge/AWS-0d1117?style=flat-square&logo=amazonaws&logoColor=FF9900)
![Docker](https://img.shields.io/badge/Docker-0d1117?style=flat-square&logo=docker&logoColor=2496ED)

</details>

<details>
<summary><b>Developer Tools</b></summary><br/>

![Git](https://img.shields.io/badge/Git-0d1117?style=flat-square&logo=git&logoColor=F05032)
![Linux](https://img.shields.io/badge/Linux-0d1117?style=flat-square&logo=linux&logoColor=FCC624)
![Anaconda](https://img.shields.io/badge/Anaconda-0d1117?style=flat-square&logo=anaconda&logoColor=44A833)
![Postman](https://img.shields.io/badge/Postman-0d1117?style=flat-square&logo=postman&logoColor=FF6C37)

</details>

<br/>

![](https://raw.githubusercontent.com/andreasbm/readme/master/assets/lines/rainbow.gif)

<br/>

## 🌐 Open Source

<div align="center">

Open to **research collaborations**, **open-source contributions**, **hackathons**, and interesting AI/NLP projects.
If you're working on Transformers, NMT, or LLM tooling — my inbox is open.

</div>

<br/>

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:1a1533,100:0d0f1a&height=3&width=100%" width="100%"/>

<br/>

## 🎲 Fun Facts

- ⚡ I enjoy implementing research papers from scratch rather than just using pretrained versions.
- 📚 I care more about *why* an algorithm works than about using it as a black box.
- 🧮 Qualified GATE CSE **and** GATE DA in the same year — now grinding for a sharper 2027 rank.
- ♟️ Chess enthusiast — peak rating 1901. [chess.com: KyaYaarYash](https://www.chess.com/member/KyaYaarYash) · [lichess: yashhaha](https://lichess.org/@/yashhaha)
- 🏏 Cricket fan.

<br/>

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:1a1533,100:0d0f1a&height=3&width=100%" width="100%"/>

<br/>

## 🤝 Connect

<div align="center">

<a href="https://github.com/yash-mishraa"><img src="https://img.shields.io/badge/-GitHub-0d1117?style=for-the-badge&logo=github&logoColor=ffffff" /></a>
<a href="https://www.linkedin.com/in/yash-mishra-700b2333b/"><img src="https://img.shields.io/badge/-LinkedIn-0d1117?style=for-the-badge&logo=linkedin&logoColor=0A66C2" /></a>
<a href="mailto:yashmishra1408@gmail.com"><img src="https://img.shields.io/badge/-Gmail-0d1117?style=for-the-badge&logo=gmail&logoColor=EA4335" /></a>

</div>

<br/>

<div align="center">
<img src="https://komarev.com/ghpvc/?username=yash-mishraa&style=flat-square&color=7C3AED&label=PROFILE+VIEWS" />
</div>

<br/>

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0d0f1a,50:1a1533,100:0d0f1a&height=110&section=footer" width="100%"/>
